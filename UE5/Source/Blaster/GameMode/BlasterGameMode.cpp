...
void ABlasterGameMode::BeginPlay()
{
    ...
    // Set the match duration for all existing PlayerStates
    for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; ++It)
    {
        APlayerState* PlayerState = It->Get()->PlayerState;
        if (PlayerState && PlayerState->IsA<ABlasterPlayerState>())
        {
            SetPlayerStateMatchDuration(Cast<ABlasterPlayerState>(PlayerState));
        }
    }
}

void ABlasterGameMode::SetPlayerStateMatchDuration(ABlasterPlayerState* PlayerState)
{
    if (PlayerState)
    {
        PlayerState->SetMatchDuration(MatchDuration);
    }
}

void ABlasterGameMode::SetMatchDuration(float Duration)
{
    MatchDuration = Duration;
}

void ABlasterGameMode::SetPlayerStateReference(ABlasterPlayerState* PlayerState)
{
    if (PlayerState)
    {
        PlayerState->GameMode = this;
    }
}
