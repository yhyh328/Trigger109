...
void ABlasterCharacter::OnTakeDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatorController, AActor* DamageCauser)
{
    Health = FMath::Clamp(Health - Damage, 0.f, MaxHealth);
    UpdateHUDHealth();
    PlayHitReactMontage();
...
void ABlasterCharacter::ServerMoveInputActionTriggered_Implementation(const FInputActionValue& Value)
{
    MoveInputActionTriggered(Value);
}

bool ABlasterCharacter::ServerMoveInputActionTriggered_Validate(const FInputActionValue& Value)
{
    return true; // 추가 검증 로직이 필요한 경우 여기에 작성
}
...
void ABlasterCharacter::PlayReloadMontage() const
{
    if (IsValid(ReloadMontage) && Combat && Combat->EquippedWeapon)
    {
        ...
        if (const auto AnimInstance = GetMesh()->GetAnimInstance(); IsValid(AnimInstance))
        {
            ...
            else if (EWeaponType::GrenadeLauncher == WeaponType)
            {
...
void ABlasterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
    ...
    if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
    {
        ...
        // Moving
        EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::MoveInputActionTriggered);

        // Looking
        EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::LookInputActionTriggered);

        EnhancedInputComponent->BindAction(EquipAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::EquipInputActionTriggered);


        EnhancedInputComponent->BindAction(ReloadAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::ReloadInputActionTriggered);


        EnhancedInputComponent->BindAction(ThrowGrenadeAction, ETriggerEvent::Triggered, this, &ABlasterCharacter::ThrowGrenadeInputActionTriggered);


        EnhancedInputComponent->BindAction(CrouchAction, ETriggerEvent::Started, this, &ABlasterCharacter::OnCrouchInputActionStarted);


        EnhancedInputComponent->BindAction(CrouchAction, ETriggerEvent::Completed, this, &ABlasterCharacter::OnCrouchInputActionCompleted);


        EnhancedInputComponent->BindAction(AimAction, ETriggerEvent::Started, this, &ABlasterCharacter::OnAimInputActionStarted);


        EnhancedInputComponent->BindAction(AimAction, ETriggerEvent::Completed, this, &ABlasterCharacter::OnAimInputActionCompleted);

        EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Started, this, &ABlasterCharacter::OnFireInputActionStarted);


        EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Completed, this, &ABlasterCharacter::OnFireInputActionCompleted);
